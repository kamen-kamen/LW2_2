...
Вариации: Если заказов еще нет, таблица заказов будет пустой до этого действия.
...
Вариации: Попробуйте ввести отрицательное количество (например, "-1") — приложение может не проверить это (на основе кода, валидация только на ID), так что заказ добавится, но это может быть багом; ожидаем добавления с отрицательным значением, что некорректно.
...
Вариации: Если изменить на невалидные данные (например, пустой стол), приложение может принять (нет строгой валидации в коде), но ожидаем обновление без ошибок.
...
Вариации: Если удалить все заказы, таблица станет пустой; проверьте, что дальнейшие действия (например, расчет счета) не крашат приложение.
...
Вариации: Выберите заказ с несуществующим блюдом (если возможно) — ожидаем ошибку в расчете (код ищет цену по имени, если не найдет — возможно 0).
...
Таблицы не изменятся.
Вариации: Если заказов нет — ожидаем пустой список или ничего (код вернет пустой QStringList).
...
Вариации: Нажмите повторно — сортировка сохранится (код сортирует ascending, без toggle).
...
Вариации: Сохраните в существующий файл — перезапишет без предупреждения (QFileDialog default).
...
Вариации: Загрузите файл с невалидным форматом — возможен краш или частичная загрузка (код не проверяет строго).
...
Вариации: Аналогично для удаления или счета — просто игнор (без предупреждения).
